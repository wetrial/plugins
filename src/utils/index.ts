 ...
 * @param pageMode {string} 模式(src|es|lib)
 */
export function getModuleRoutes(module: string, absRootPath: string, pageMode: string) {
  // 对应到lib文件夹下
  ...
  const moduleAbsPath = getModuleAbsolutePath(module, absRootPath, pageMode);
 ...
 */
function getModuleAbsolutePath(module: string, absRootPath: string, pageMode: string) {
  // 模块可能是相对路径写法或者模块包的写法
  let moduleAbsPath: string;
  if (module.startsWith('.')) {
    moduleAbsPath = resolve(absRootPath, module, pageMode);
  } else {
    // 模块形式(定到lib模块下)
    moduleAbsPath = resolve(absRootPath, `./node_modules/${module}/${pageMode}`);
  }
